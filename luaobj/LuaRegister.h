...
namespace LCD
{
	...
	template <typename Callee,typename Func>
	class DirectConstructorDispatchHelper
	{
	public:
		static int DirectConstructorDispatcher(lua_State* L)
		{
 			unsigned char* buffer = GetFirstUpValueAsUserData(L);
			return Call(*(Func*)(buffer), L, 1);
		}
	...
	template <typename T>
	class DirectDestructorDispatchHelper
	{
		typedef T * ptr_t;
	public:
		static int DirectDestructorDispatcher(lua_State* L)
		{
 			ptr_t *ud = static_cast<ptr_t*>(lua_touserdata(L, 1));
			if(ud)
			{
				delete *ud;
				*ud=NULL;
...
public:
	...
	~LuaRegister()
	{
		assert(m_index==lua_gettop(m_lstate));
		lua_pushlightuserdata(m_lstate,static_cast<void*>(&s_key));
		lua_pushvalue(m_lstate,m_index);
		lua_settable(m_lstate, LUA_REGISTRYINDEX);
		lua_pop(m_lstate,1);//pop table
	}

	template<typename Func>
	void Register(const char* name,Func func)
	{
		unsigned char* buffer = (unsigned char*)lua_newuserdata(m_lstate, sizeof(func));
		memcpy(buffer, &func, sizeof(func));
		lua_pushcclosure(m_lstate,LCD::DirectCallObjectMemberDispatcherHelper<T, Func, 1>::DirectCallMemberDispatcher,1);
		lua_setfield(m_lstate,m_index,name);
	}

	template <typename Func>
	void RegisterStatic(const char* name,Func func)
	{
		unsigned char* buffer = (unsigned char*)lua_newuserdata(m_lstate, sizeof(func));
		memcpy(buffer, &func, sizeof(func));
		lua_pushcclosure(m_lstate,LCD::DirectCallFunctionDispatchHelper<Func>::DirectCallFunctionDispatcher,1);
		lua_setfield(m_lstate,m_index,name);
	}


	template <typename Func>
	void RegisterNew(const char* name,Func func)
	{
		unsigned char* buffer = (unsigned char*)lua_newuserdata(m_lstate, sizeof(func));
		memcpy(buffer, &func, sizeof(func));
		lua_pushcclosure(m_lstate,LCD::DirectConstructorDispatchHelper<T,Func>::DirectConstructorDispatcher,1);
		lua_setfield(m_lstate,m_index,name);
	}

	void RegisterGC()
	{
		lua_pushcfunction(m_lstate, LCD::DirectDestructorDispatchHelper<T>::DirectDestructorDispatcher);
		lua_setfield(m_lstate,m_index,"__gc");
	}
	...
	{
		return (ptr_t)obj.toData();;
	}

	static LuaObject cToLua(LuaState* L,T* obj)
	{
		ptr_t *ud = static_cast<ptr_t*>(lua_newuserdata(L, sizeof(ptr_t)));
		*ud = obj;  // store pointer to object in userdata
		lua_getglobal(L,m_className);  // lookup metatable in Lua registry
		lua_setmetatable(L, -2);	
		return LuaObject(L,L->getTop());
	}
